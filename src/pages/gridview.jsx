...
function GridView() {
  ...
  const filteredProducts =
      ...
      selectedFilters.priceRange[1] === 999999
      ? [] // Show nothing initially
      : products.filter(prod => {
        const matchCategory =
          selectedFilters.categories.length === 0 ||
          selectedFilters.categories.includes(prod.category);

        const matchBrand =
          selectedFilters.brands.length === 0 ||
          selectedFilters.brands.includes(prod.brand);

        const matchFeature =
          selectedFilters.features.length === 0 ||
          (Array.isArray(prod.features) &&
            selectedFilters.features.every(f =>
              prod.features.includes(f)
            ));

        const matchRating =
          selectedFilters.rating.length === 0 ||
          selectedFilters.rating.includes(Math.floor(prod.rating));

        const matchCondition =
          selectedFilters.condition === null ||
          selectedFilters.condition === prod.condition;

        const matchPrice =
          prod.price >= selectedFilters.priceRange[0] &&
          prod.price <= selectedFilters.priceRange[1];
  ...
  return (
    <>
      ...
      <div className='max-w-7xl mx-auto mt-6 flex flex-col items-start  gap-2 px-4 '>
        ...
        <p className='hidden  text-stone-400 text-lg font-light mb-3  sm:flex gap-2 items-center'>
        ...
        <div className='sm:flex hidden items-start p-2 w-full justify-between gap-2'>
          ...
          <div className='w-1/4'>
            ...
            <div className='border-t border-gray-300 pt-2'>
              ...
              {showRating && [1, 2, 3, 4, 5].map((stars) => (
          ...
          <div className='w-3/4'>
            ...
            <div className='mb-3  flex justify-between items-center p-3 rounded w-full border border-stone-300'>
              ...
              <div className='flex items-center gap-3 justify-between'>
                ...
                <div className='flex items-center gap-2'>
                  ...
                  <p className='text-xl text-stone-500'>Verified Only</p>
                </div>

                <div onClick={() => { setshowsorting(!showsorting) }} className="flex gap-2 items-center h-5 cursor-pointer relative border border-stone-400 rounded py-4 px-2 ">
                  <a href="#" className='text-stone-500 font-bold text-xl'>Sorting: <span className='text-lg font-medium'>{selectedsort}</span></a>
                  {showsorting ? <IoIosArrowUp className="text-stone-400 text-lg" /> : <IoIosArrowDown className="text-stone-400 text-lg" />}
                  {showsorting &&
            ...
            {showgrid &&
              <div className='grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4'>
                ...
                {getCurrentProducts().map((prod, i) => (
                  <div key={i} className='border border-stone-300 rounded-md shadow-sm p-4'>
            ...
            {showlist &&
              <div className='flex flex-col gap-3'>
                {getCurrentProducts.map((prod, i) => (
                  <div key={i} className='flex items-start gap-4 border border-stone-300 rounded-md shadow-sm p-4 bg-white'>
            ...
            <div className='flex items-center  justify-end gap-4  mt-6 p-2'>
              ...
              <select className='border border-stone-300 px-3 py-1 rounded-md cursor-pointer text-stone-500 outline-none p-6'
                onChange={(e) => {
